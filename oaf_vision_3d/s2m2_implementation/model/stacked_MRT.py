...
from oaf_vision_3d.s2m2_implementation.model.attentions import (
    BasicAttnBlock,
    GlobalAttnBlock,
)
...
class MRT(nn.Module):
    ...
    def __init__(
        self, dims: list, num_heads: int, dim_expansion: int, use_gate_fusion: bool
    ):
        ...
        self.down_conv0 = nn.Sequential(
            nn.AvgPool2d(2), nn.Conv2d(dims[0], dims[1], kernel_size=1)
        )
        self.down_conv1 = nn.Sequential(
            nn.AvgPool2d(2), nn.Conv2d(dims[1], dims[2], kernel_size=1)
        )
        self.down_conv2 = nn.Sequential(
            nn.AvgPool2d(2), nn.Conv2d(dims[2], dims[2], kernel_size=1)
        )
        ...
        self.down_concat1 = FeatureFusion(
            dims[1], kernel_size=1, use_gate=use_gate_fusion
        )
        self.down_concat2 = FeatureFusion(
            dims[2], kernel_size=1, use_gate=use_gate_fusion
        )
        self.down_concat3 = FeatureFusion(
            dims[2], kernel_size=1, use_gate=use_gate_fusion
        )

        self.up_concat0 = FeatureFusion(
            dims[0], kernel_size=1, use_gate=use_gate_fusion
        )
        self.up_concat1 = FeatureFusion(
            dims[1], kernel_size=1, use_gate=use_gate_fusion
        )
        self.up_concat2 = FeatureFusion(
            dims[2], kernel_size=1, use_gate=use_gate_fusion
        )

        self.enc_attn0 = BasicAttnBlock(
            dim=dims[0],
            num_heads=1 * num_heads,
            dim_expansion=dim_expansion,
            use_pe=False,
        )
        self.enc_attn1 = BasicAttnBlock(
            dim=dims[1],
            num_heads=2 * num_heads,
            dim_expansion=dim_expansion,
            use_pe=False,
        )
        self.enc_attn2 = BasicAttnBlock(
            dim=dims[2],
...
class StackedMRT(nn.Module):
    ...
    def forward(self, z0: Tensor, z1: Tensor, z2: Tensor, z3: Tensor) -> Tensor:
        for _, uformer in enumerate(self.uformer_list):
