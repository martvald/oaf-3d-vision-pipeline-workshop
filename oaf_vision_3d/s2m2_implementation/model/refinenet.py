...
class ConvGRU(nn.Module):
    def __init__(
        self, hidden_dim: int = 128, input_dim: int = 128, kernel_size: int = 3
    ) -> None:
        super().__init__()
    ...
    def forward(self, h: Tensor, x: Tensor) -> Tensor:
        ...
        q = F.tanh(self.convq1(torch.cat((r * h, x), axis=1)))  # type: ignore[call-overload]
        h = (1 - z) * h + z * q

        hx = torch.cat([h, x], dim=1)
        z = torch.sigmoid(self.convz2(hx))
        r = torch.sigmoid(self.convr2(hx))
        q = F.tanh(self.convq2(torch.cat([r * h, x], axis=1)))  # type: ignore[call-overload]
        h = (1 - z) * h + z * q
...
class GlobalRefiner(nn.Module):
    ...
    def __init__(self, feature_channels: int) -> None:
        ...
        self.refine_unet = Unet(
            dims=[feature_channels, 1 * feature_channels, 1 * feature_channels],
            dim_expansion=1,
            use_pe=False,
...
class LocalRefiner(nn.Module):
    """Refine disparity in local iterative manner using local feature
    correlations."""
    ...
    def __init__(
        ...
        self.corr_feat1 = nn.Sequential(
            nn.Conv2d((2 * radius + 1) * 1, 96, kernel_size=1),
            nn.GELU(),
        ...
        self.disp_update = nn.Sequential(
            nn.Conv2d(feature_channels, 1 * feature_channels, kernel_size=3, padding=1),
            nn.GELU(),
    ...
    def forward(
        ...
        conf: Tensor,
        occ: Tensor,
        cv_fn: Callable[[Tensor], tuple[Tensor, Tensor]],
        ...
        disp_corr_ctx_feat = self.disp_corr_ctx_cat(
            torch.cat([disp_feat, corr_feat1, corr_feat2, ctx, conf_feat], dim=1).to(
                disp.dtype
            )
        )
        ...
        return (
            ...
            disp_new.to(disp.dtype),
            conf_new.to(disp.dtype),
            occ_new.to(disp.dtype),
        )
